...
export default function Gallery() {
  ...
  const activeList = galleryMode === "3d" ? memoriesData : filteredMemories;

  // Currently selected memory for the lightbox
  const selectedMemory =
    selectedIndex !== null && selectedIndex >= 0 && selectedIndex < activeList.length
  ...
  const handleNext = () => {
    if (selectedIndex !== null && selectedIndex < activeList.length - 1) {
      setSelectedIndex(selectedIndex + 1);
    }
  };
  ...
  return (
    <section id="galeri" className="py-24 px-4 bg-neo-yellow border-b-4 border-black relative">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Dual Mode Switcher Bar: 3D Museum vs 2D Grid */}
        <div className="flex justify-center items-center mb-12">
          <div className="inline-flex p-1.5 bg-black border-4 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] gap-2">
            <button
              type="button"
              onClick={() => {
              ...
              className={`inline-flex items-center gap-2.5 px-4 sm:px-7 py-3 font-black text-sm sm:text-base uppercase tracking-wider transition-all duration-200 cursor-pointer ${
                galleryMode === "3d"
                  ? "bg-neo-yellow text-black shadow-[2px_2px_0px_0px_#000] -translate-y-0.5"
                  : "bg-black text-white hover:text-neo-yellow"
              }`}
            >
              <Landmark className="w-5 h-5 stroke-[2.5]" />
              <span>Mode Museum 3D</span>
              <span className="hidden sm:inline-block text-[10px] bg-red-500 text-white px-2 py-0.5 font-black uppercase tracking-wider border border-black">
                Virtual Art
              </span>
            </button>
            <button
            ...
            >
              ...
              <span>Mode Grid 2D</span>
              <span className="hidden sm:inline-block text-[10px] bg-neutral-700 text-neo-white px-2 py-0.5 font-mono border border-neutral-600">
                {memoriesData.length} Foto
              </span>
            </button>
          </div>
        </div>
          ...
          {/* Category Filter Bar */}
              ...
              <div className="relative">
                ...
                {searchQuery && (
                  <button
                    ...
                    className="absolute right-3 top-1/2 -translate-y-1/2 bg-black text-neo-white p-1 border border-black hover:bg-red-500 transition-colors cursor-pointer"
            ...
            {filteredMemories.length > visibleCount && (
              <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mt-12">
                ...
                >
                  + Muat {ITEMS_PER_LOAD} Foto Lagi ({filteredMemories.length - visibleCount} Foto Tersisa)
                </button>
                <button
                  type="button"
              ...
              <div className="flex items-center justify-between px-4 py-3 bg-neo-yellow border-b-4 border-black">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-mono font-bold text-neutral-800 ml-2 hidden sm:inline-block">
                    ({(selectedIndex ?? 0) + 1} / {activeList.length})
                  </span>
                </div>
                <button
                  type="button"
                  onClick={() => setSelectedIndex(null)}
                  className="inline-flex items-center gap-1.5 bg-black text-neo-white hover:bg-red-500 hover:text-white px-3 py-1 font-black text-sm uppercase border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-colors cursor-pointer"
                >
              ...
              <div className="relative flex-1 overflow-auto p-4 sm:p-6 bg-neutral-900 flex items-center justify-center min-h-[320px]">
                ...
                {selectedIndex !== null && selectedIndex > 0 && (
                  <button
                    ...
                    className="absolute left-4 top-1/2 -translate-y-1/2 bg-neo-yellow hover:bg-white text-black font-black p-3 sm:p-3.5 border-4 border-black shadow-[4px_4px_0px_0px_#000] z-20 cursor-pointer"
                ...
                {/* Next Button */}
                {selectedIndex !== null && selectedIndex < activeList.length - 1 && (
                  <button
                    type="button"
                    onClick={(e) => {
                      e.stopPropagation();
                    ...
                    className="absolute right-4 top-1/2 -translate-y-1/2 bg-neo-yellow hover:bg-white text-black font-black p-3 sm:p-3.5 border-4 border-black shadow-[4px_4px_0px_0px_#000] z-20 cursor-pointer"
